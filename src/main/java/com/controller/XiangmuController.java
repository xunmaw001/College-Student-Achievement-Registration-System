...
//            xiangmu.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R shenhe(@RequestBody XiangmuEntity xiangmu, HttpServletRequest request){
        logger.debug("shenhe方法:,,Controller:{},,xiangmu:{}",this.getClass().getName(),xiangmu.toString());

//        if(xiangmu.getXiangmuYesnoTypes() == 2){//通过
//            xiangmu.setXiangmuTypes();
//        }else if(xiangmu.getXiangmuYesnoTypes() == 3){//拒绝
//            xiangmu.setXiangmuTypes();
//        }
        xiangmu.setXiangmuShenheTime(new Date());//审核时间
        xiangmuService.updateById(xiangmu);//审核
        return R.ok();
    }
